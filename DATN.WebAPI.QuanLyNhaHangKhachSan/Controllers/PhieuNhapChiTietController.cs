...
{   [Authorize]
    ...
    public class PhieuNhapChiTietController : Controller
    {
        ...
        [HttpPost]
        [Route("Them-PNCT")]
        public async Task<ActionResult<PhieuNhapChiTiet>> ThemPhieuNhapCHiTiet(PhieuNhapChiTietDTO phieuNhaoCT)
        {
            ResponseDTO responseDTO = new ResponseDTO();
            try
            {
                ErrorMessageDTO error = await phieuNhapChiTietDAO.ThemPhieuNhapChiTiet(phieuNhaoCT);
